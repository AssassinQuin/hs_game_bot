...
```text
──── 3f9a12cd · 我的第2回合开始 (T4) │ 水晶 4/4 ────
[3f9a12cd·T4·我] 打出 滋养 (5费→实付3费) 剩2费
[3f9a12cd·T4·我] 抽到 野性成长
[3f9a12cd·T5·对面] 攻击 野兽 → 我的英雄
── T5 3f9a12cd(回合结束) 我:水晶7/10 │ 手牌6 │ 牌库18 │ 场上2 │ 对面场攻1 ──
════════ 完整快照 ════════ 回合T5(我的第3回合·回合结束) │ 3f9a12cd │ …
  …… 双方手牌/场面/英雄状态 + 组件台账(差1张组件还剩几抽) + 本回合出牌链路 ……
──── 对局结束 ──── 湫然#51704=WON / 对手=LOST
```

- **对局 hash**：每局开头算出 8 位 hash（会话名+开局时刻，`3f9a12cd`），
  标题/链路行首/快照/训练 jsonl `_meta.game_id` 四方同源 —— 控制台输出的
  任何一句都能凭它对回 `data/training/` 的 jsonl 与 `.power.log` 切片；
  局号是 bot 本次运行内的计数器，重启即漂移，故不做合并主键。
