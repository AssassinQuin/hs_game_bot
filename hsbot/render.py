...
def chain_line(evt: dict, carddb: CardDB) -> str:
    ...
    if kind == "attack":
        ...
        atk = (hero_txt(evt.get("attacker_card_id")) if evt.get("attacker_is_hero")
...
def _board_txt(gs: GameState, key, carddb: CardDB) -> str:
    cells = []
    for e in gs.board(key):
        t = "嘲讽" if e.taunt else ""
        cells.append(f"{carddb.name(e.card_id)}({e.atk}/{e.hp_total}{t})")
...
def snapshot_block(gs: GameState, led: Ledger | None, *, knowledge: DeckKnowledge | None,
                   deck_name: str, generic: bool, game_no: int,
                   chain_lines: list[str], chain_summary: list[str],
                   carddb: CardDB, reason: str) -> str:
    me, opp = gs.friendly_key, gs.opponent_key()
    generic = generic or knowledge is None
    mode = gs.meta.get("GameType", "?")
    fmt = gs.meta.get("FormatType", "")
    reason_cn = _REASON_CN.get(reason, reason)
    lines: list[str] = []

    ft = gs.friendly_turn_number() if me is not None else 0
    flag = " │ [通用模式: 非所选卡组]" if generic else ""
    lines.append(f"════════ 完整快照 ════════ 回合T{gs.turn}(我的第{ft}回合·{reason_cn})"
    ...
    if me is not None:
        ...
        lines.append(f"我  {gs.name(me)} [{hname}] {gs.hero_hp(me)}血{gs.hero_armor(me)}甲"
                     f" │ 水晶 {gs.mana_now(me)}/{f['res']} │ 牌库{gs.deck_count(me)}"
                     f" │ 本回合已用{f['used']}费")
        lines.append(f"    下一回合: {gs.mana_next_turn(me)}费{ol}")
        hs = []
        for i, e in enumerate(gs.hand(me), 1):
            ...
            gen = "[衍生]" if e.generated else ""
            hs.append(f"{i}{carddb.name(e.card_id)}({cost}费{mark}){gen}")
        lines.append(f"    手牌{len(hs)}(位置序): {' '.join(hs) if hs else '(空)'}")
        lines.append(f"    场面: {_board_txt(gs, me, carddb)}"
                     f"   墓地:{gs.graveyard_count(me)}张 奥秘:{gs.secret_count(me)}")

    if opp is not None:
        hero = gs.hero(opp)
        hname = carddb.name(hero.card_id) if hero else "?"
        of = gs.mana_fields(opp)
        opp_mana = (f" │ 水晶 {gs.mana_now(opp)}/{of['res']}"
                    if gs.current_key() == opp else
                    f" │ 下回合水晶 {gs.mana_next_turn(opp)}/{of['res']}")
        lines.append(f"对面 {gs.name(opp)} [{hname}] {gs.hero_hp(opp)}血{gs.hero_armor(opp)}甲"
                     f"{opp_mana} │ 牌库{gs.deck_count(opp)} │ 手牌{len(gs.hand(opp))}(不可见)")
        lines.append(f"    场面: {_board_txt(gs, opp, carddb)}")
...
def game_end_line(gs: GameState) -> str:
    states = " / ".join(f"{gs.name(k)}={gs.playstate(k)}" for k in gs.player_keys())
